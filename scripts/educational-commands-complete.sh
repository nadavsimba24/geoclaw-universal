...
show_capabilities() {
  ...
  print_component "n8n Integration" "Workflow automation"
  echo "  • 200+ integrations out of the box"
  echo "  • Visual workflow editor"
  echo "  • Trigger workflows from Geoclaw"
  echo "  • Commands: @geoclaw n8n execute, @geoclaw n8n create"
  echo ""
  
  print_component "QGIS/PostGIS" "Geospatial analysis"
  echo "  • Spatial queries and mapping"
  echo "  • GIS data processing"
  echo "  • Professional geospatial tools"
  echo "  • Commands: @geoclaw spatial query, @geoclaw create map"
  echo ""
  
  print_component "Web Scraping" "Data collection"
  echo "  • Scrape websites with JavaScript"
  echo "  • Navigate multi-page sites"
  echo "  • Extract structured data"
  echo "  • Commands: @geoclaw scrape, @geoclaw navigate"
  echo ""
  
  print_component "Workflow Orchestration" "Magic workflows"
  echo "  • Connect ALL components into pipelines"
...
learn_component() {
  ...
  case "$component" in
    ...
    vibe-kanban|task-management)
      ...
      echo "  3. Start workspace: @geoclaw start workspace for task 123"
